...
i2c_status i2c_recv(u8 address, u8 *buffer, u32 size) {
    ...
    while (!(REGS_I2C->status & S_DONE)) {
        while (REGS_I2C->status & S_RXD) {
            *buffer++ = REGS_I2C->fifo & 0xFF;  //only want first byte, so masked with 0xFF
            count++;
        }
    }

    while (count < size && REGS_I2C->status & S_RXD) {  // there is still data to be processed
